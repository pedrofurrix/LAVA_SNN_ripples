...
class SpikeTimePenalty(nn.Module):
    ...
    def forward(self, spk_out, targets):
        ...
        and observed spikes = 0. If so, revert the penalty factor since it is understandable that the neuron did not spike
        '''
...
def my_first_spike_acc(output, target, tolerance=0, verbose=False,only_spiked=False):
    '''
    ...
    if not only_spiked:
        ...
        # A prediction is correct if the neuron spiked within the window and target was valid
        correct_mask = (spiked_mask & valid_target_mask)| (spiked_mask==False & valid_target_mask==False)
        # Accuracy = fraction of correct predictions out of all neurons
        accuracy = torch.mean(correct_mask.float())
