...
def undersample_majority(input_data, gt, c1_mask = None,multiplier=1,mult_minority=1) -> tuple[np.ndarray, np.ndarray]:
    '''
    ...
    num_minority_to_keep=int(min(num_minority,num_minority*mult_minority))

    num_majority_to_keep = int(min(len(majority_indices), multiplier * num_minority_to_keep))
    # Shuffle and slice
    ...
    selected_minority = minority_indices[:  num_minority_to_keep]
