...
def run_inference(
    ...
    for session in tqdm(sessions):
        ...
        filtered_signal, ripples = load_experimental_data(
            session_path,
            channel=channel, # channels are 1-indexed
            load_data=True,
            downsample=False,
            normalize=True,
            offset=0.16,
        ...
        n_use = n_total // 5
        spk = spk[-n_use:, :]
        print(f"Using last {n_use} samples out of {n_total} total samples.")
        ...
        # Adjust GT to match used segment
        ripples_start = ripples_start[ripples_start >= (n_total - n_use)]
        ripples_start = ripples_start - (n_total - n_use)
