...
def process_live_results(
    ...
    for session, data in all_spikes.items():
        ...
        results.append({
            "Session": session,
            "Channel": channel,
            "Network": network_name if network_name else os.path.basename(spikes_pkl_path).replace('_spikes.pkl', ''),
            "ADAPT": adapt,
            "TP": tp,
            "FP": fp,
            "FN": fn,
...
if __name__ == "__main__":
    ...
    if all_dfs:
        ...
        # Print summary
        print("\n--- Summary by Network ---")
        summary = final_df.groupby("Network")[["F1", "Precision", "Recall", "TP", "FP", "FN"]].mean()
        print(summary)
    else:
