...
from dutta_test.process_signal import load_experimental_data
...
def process_live_results(
    ...
    print(f"Loading spikes from: {spikes_pkl_path}")
    ...
    for session, data in all_spikes.items():
        # Get channel from lists_sessions
        channel = lists_sessions.channel_sessions.get(session)
        if channel is None:
            print(f"Warning: Channel not found for session {session}")
            continue
            
        # Extract spikes from detections list of dicts
        # detections: [{"sample": int, "time_s": float}, ...]
...
if __name__ == "__main__":
    ...
    SPIKES_ROOT = os.path.join(curr_dir, "spikes")
    ...
    OUTPUT_FILE = os.path.join(SPIKES_ROOT, "all_networks_metrics_dutta.csv")
    ...
    for root, dirs, files in os.walk(SPIKES_ROOT):
        for file in files:
            if file.endswith(".pkl") and "detections" in file:
                ...
                adapt = float(match.group(1)) if match else 0
                ...
                try:
                    df = process_live_results(
                        ...
                        data_path_extra=DATA_PATH_EXTRA,
                        tolerance=20,
                        max_detection_offset=100,
    ...
    if all_dfs:
        ...
        summary = final_df.groupby("Threshold")[["F1", "Precision", "Recall", "TP", "FP", "FN"]].mean()
        print(summary)
    else:
        print("No spike pickle files found.")
