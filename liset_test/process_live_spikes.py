...
def process_live_results(
    ...
    for session, data in all_spikes.items():
        ...
        # Extract spikes from detections dict
        # runner.py saves: {"pred_indexes": ..., "pred_times": ...}
        if 'pred_times' in data:
            spikes = data['pred_times'] * 1000 # Convert s to ms
        else:
...
if __name__ == "__main__":
    ...
    if not os.path.exists(SPIKES_ROOT):
        print(f"Directory not found: {SPIKES_ROOT}")
        sys.exit(1)
